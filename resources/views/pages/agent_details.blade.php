...
<!--Breadcrumb Section-->
  <section class="breadcrumb-box" data-parallax="scroll" data-image-src="@if(getcong('title_bg')) {{ URL::asset('upload/'.getcong('title_bg')) }} @else {{ URL::asset('site_assets/img/breadcrumb-bg.jpg') }} @endif">
    <div class="inner-container container">
      <h1>@if($agent->usertype=='Agents' or $agent->usertype=='Admin') Agent @else Owner @endif Details</h1>
      <div class="breadcrumb">
        <ul class="list-inline">
          <li class="home"><a href="{{ URL::to('/') }}">Home</a></li>
          @if($agent->usertype=='Agents')<li><a href="{{ URL::to('agents/') }}">Agents</a></li>@endif
          <li> {{$agent->name}}</li>
        </ul>
      </div>
    </div>
  ...
  <section class="main-container agent-box-container container" style="padding-top: 40px;">
    ...
    <div class="property-listing-container clearfix">
      <div class="title-box clearfix">
        <h2 class="hsq-heading type-1">@if($agent->usertype=='Agents' or $agent->usertype=='Admin') Agent's @else Owner's @endif Properties</h2>
      ...
      <div class="property-container clearfix">
        ...
        <div class="propertybox-featured col-md-6">
          <div class="inner-container clearfix">
            <a href="{{ url('properties/'.$property->property_slug.'/'.Crypt::encryptString($property->id)) }}" class="img-container col-xs-6">
            ...
            <div class="col-xs-6 main-info">
              ...
              <div class="bottom-sec clearfix">
                ...
                <a href="{{ url('properties/'.$property->property_slug.'/'.Crypt::encryptString($property->id)) }}" class="btn more-link">More Info</a>
              </div>
            </div>
          </div>
        </div>
